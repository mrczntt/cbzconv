...
for input_file in args.input_files:
	...
	match extension.upper():
		...
		case '.CBR':
			unarchiver_path = shutil.which('unrar')
			if unarchiver_path is not None:
				subprocess.run([unarchiver_path, 'e', input_file, temp_dir], stdout=subprocess.DEVNULL)
			else:
				print('Cannot find unrar in PATH. Skipping.')
				continue
		case '.CB7':
			unarchiver_path = shutil.which('7z')
			if unarchiver_path is not None:
				subprocess.run([unarchiver_path, 'e', input_file, '-o' + temp_dir], stdout=subprocess.DEVNULL)
			else:
				print('Cannot find 7z in PATH. Skipping.')
				continue
		case other:
